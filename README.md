...
Since we all know, unless you are working on a big, complex project with lots of branches and collaborations, you will likely use a single shared remote repository with the default name `origin`. 
...
**Git for Humans** is focusing on these 20% commands that you will most likely touch in an everyday use. For more complex use, you can always fallback to the original Git commands. [All Git commands](https://git-scm.com/docs) is applicable in **Git for Humans** (e.g., you can use `git fetch` and `g fetch` interchangeably).
